...
class JobManager(models.Manager):
    def job_validator(self, postData):
        ...
        if len(postData["title"]) < 3:
            job_errors["title"] = "Title must be at least 3 chars!"
            
        if len(postData["title"]) == None:
            job_errors["title"] = "Title must be provided!"
        
            
        if len(postData["description"]) < 3:
            job_errors["description"] = "Description be at least 3 chars!"
            
        if len(postData["description"]) == None:
            job_errors["description"] = "Description must be provided!"
            
        if len(postData["location"]) < 3:
            job_errors["location"] = "Location must be at least 3 chars!"
            
        if len(postData["location"]) == None:
            job_errors["location"] = "Location must be provided!"
...
class Job (models.Model):
    ...
    def __repr__(self):
        return f"User: {self.id} {self.title} {self.location}"
